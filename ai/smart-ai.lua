...
sgs.ai_compare_funcs = {
	...
	defense = function(a,b)
		...
		if (a:getHandcardNum()<2) and (b:getHandcardNum()>=2) then return false end
		if (b:getHandcardNum()<2) and (a:getHandcardNum()>=2) then return true end
		
                if sgs.rebel_target:objectName()==a:objectName() then return false end
                if sgs.rebel_target:objectName()==b:objectName() then return true end
                
                if sgs.loyal_target then
                    if sgs.loyal_target:objectName()==a:objectName() then return false end
                    if sgs.loyal_target:objectName()==b:objectName() then return true end
                end
...
--            else modifier=2
    ...
    elseif self.role=="renegade" then
        ...
        if not hasRebel then 
			if player:isLord() then return 1 
			else return 5 end
		end
		...
		elseif rebel_num < loyalish_num then
			...
			elseif player:isLord() then return -1
			end
		else
			...
			if loyalish_hp/loyalish_count <= rebel_hp/rebel_count then
				...
				if player:getRole() == "rebel" then return 3
					...
					elseif loyalish_count > 1 then
						if player:isLord() then return -1 else return 5 end
					end
				end
			end
		end
...
function SmartAI:askForYiji(cards)									----complete this function
...
function SmartAI:slashProhibit(card,enemy)
	...
	if enemy:getArmor() and (enemy:getArmor():objectName() == "renwang_shield") and card:isBlack() then					--renwang shield
		if not self.player:getWeapon() or self.player:getWeapon():objectName() ~= "qinggang_sword"  then
			return true
		end	
	end	
	
	if enemy:getArmor() and (enemy:getArmor():objectName() == "vine") and card:inherits("NatureSlash") then					--problem about equipping vine with nature slash
		return true
	end	
...
function SmartAI:useBasicCard(card, use,no_distance)
        ...
        if card:inherits("Slash") and self:slashIsAvailable() then
			...
			for _, friend in ipairs(self.friends_noself) do						----guojia with is friend
				...
				if friend:hasSkill("yiji") and friend:getLostHp() <1 then
...
function SmartAI:useCardDismantlement(dismantlement, use)
	...
	for _, friend in ipairs(friends) do
		if friend:containsTrick("indulgence") or friend:containsTrick("supply_shortage") 
			and not friend:hasSkill("wuyan") then								---·ÇÐìÊü
			use.card = dismantlement
                        if use.to then use.to:append(friend) end
	...
	for _, enemy in ipairs(enemies) do
		...
		--if equips or not (enemy:hasSkill("kongcheng") or enemy:hasSkill("lianying")) then			--not all conditions
		    ...
		    if  not enemy:isNude() and not enemy:hasSkill("wuyan") and					---update
			   (not enemy:hasSkill("xiaoji") or enemy:getEquips():isEmpty()) and							
			   (not enemy:hasSkill("kongcheng") or not enemy:getHandcardNum() == 1) and
			   (not enemy:hasSkill("lianying") or not enemy:getHandcardNum() == 1) then                   
				use.card = dismantlement
                if use.to then use.to:append(enemy) end
				return 
		    end
...
function SmartAI:useCardSnatch(snatch, use)
	...
	for _, enemy in ipairs(enemies) do
		if  not enemy:isNude() and not enemy:hasSkill("wuyan") and
			   ...
			   (enemy:hasSkill("lianying") and enemy:getHandcardNum() == 1) then                   
				use.card = snatch
                if use.to then use.to:append(enemy) end
				return
		end
...
function SmartAI:useCardDuel(duel, use)
	...
	for _, enemy in ipairs(enemies) do
		...
		if n1 >= n2  and not enemy:hasSkill("wuyan") then													--no xushu
			use.card = duel
                        if use.to then use.to:append(enemy) end
...
function SmartAI:getKeepValue(card,kept)
	...
	local class_name = card:className()										---change keepvalue
        local value	
      --  if sgs[self.player:getGeneralName().."_keep_value"] then							
       --     value=sgs[self.player:getGeneralName().."_keep_value"][class_name]
		...
		if self.player:hasSkill("qingguo") then 
			if card:inherits("Slash") then
				value = 4.2
		...
		if self.player:hasSkill("guidao") then
			if card:inherits("Jink") then value = 4.9
			elseif card:getSuit() == sgs.Card_Spade then value = 4.8
			end
		end	
		...
		if self.player:hasSkill("qiangxi") then
			if card:inherits("Weapon") then
				value = 4.1
			end
		end	
...
function SmartAI:askForCard(pattern,prompt)
        ...
        if parsedPrompt[1]=="collateral-slash" then return "."						--must add codes here
		...
		end																	--guojia don't use jink when not wounded 
		if self.player:hasSkill("tianxiang") then							--tianxiang 
			local cards = self.player:getCards("h")
			for _, card in sgs.qlist(cards) do
				if card:getSuit() == sgs.Card_Heart or card:getSuit() == sgs.Card_Spade then
					local players = self.room:getOtherPlayers(self.player)
					players = QList2Table(players)
					for _, other in ipairs(players) do
						if ((other:getLostHp() == 0 or other:getHp() == 1) and self.player:isEmeny(other)) or 
							(other:getHandcardNum() <= 2 and other:getLostHp() < 2 and self.player:isFriend(other)) then	
							return "."
						end
					end
				end	
...
function SmartAI:askForNullification(trick_name, from, to, positive)   							-----add nullification
	...
	local null_card
		for _, card in sgs.qlist(cards) do
			...
			if card:isBlack() and self.player:hasSkill("kanpo") then
		        ...
		        local null_card = sgs.Card_Parse(card_str)				
				break
			end
		end
	...
	if positive then
		...
		if null_card then return null_card end
	end
...
function SmartAI:fillSkillCards(cards)
    for _,skill in ipairs(sgs.ai_skills) do
        if skill.name=="xianzhen" then self:log("xianzhen") end
        if self:hasSkill(skill) then    
...
function SmartAI:cardNeed(card)
	if self.player:hasSkill("qingnang") then					---add needcard for godsalvation
		if card:isRed() then return 10 end
	end 	
	if (self.player:hasSkill("qixi") or self.player:hasSkill("duanliang")) and self.player:getHp() > 2 then 
		if card:isBlack() then return 10 end
	end	
	if self.player:hasSkill("tianxiang") then 
		if (card:getSuit() == sgs.Card_Spade or card:getSuit() == sgs.Card_Heart) then return 10 end
	end	
	if self.player:hasSkill("guose") and self.player:getHp() > 1 then 
		if card:getSuit() == sgs.Card_Diamond then return 10 end
	end	
	if self.player:hasSkill("jiuchi") then 
		if card:isBlack() then return 10 end
	end	
	if self.player:hasSkill("jizhi") and self.player:getHp() > 1 then
		if card:getTypeId() == sgs.Card_Trick then	return 10 end
	end
